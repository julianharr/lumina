...
def meetup_events_finder(options = {})
  ...
  p url = URI("https://api.meetup.com/find/upcoming_events?lon=#{lon}&page=20&radius=smart&lat=#{lat}&order=best&text=#{cat}&fields=featured_photo,event_hosts,group_join_info,rsvpable_after_join")
  # original below
...
def meetup_event_spooler(options = {})
  ...
  options['events'].each do |value|
    ...
    make_me = Event.new( # change to create! later
      ...
      group_rsvp_after: value['rsvpable_after_join'], # bool
    ...
    make_me.group_questions ||= []
    make_me.group_questions << questions_arr.flatten
    make_me.group_questions = make_me.group_questions.flatten

    make_me.save! if make_me.valid?
